...
def lambda_handler(event, context):
    ...
    vehical = [
        ...
        { "id": "2", "type": "motobike", "available": "false"},
    ...
    # /pets List all pets
    response_body = {}
    if (resource == "/vehicals"):
        response_body = {
            "vehicals": vehical
        }
    # /pets/petId find pet by Id    
    elif (resource == "/vehicals/{id}"):
        petId = event['pathParameters']['id']
        value = next((item for item in vehical if item["id"] == str(petId)), False)
        if( value == False ):
            err = "Vehical not found"
        else:
            response_body = {
                "vehical": value
            }
